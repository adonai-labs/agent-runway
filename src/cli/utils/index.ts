...
const CLAUDE_AGENT_RUNWAY_START = '<!-- agent-runway:claude:start -->';
const CLAUDE_AGENT_RUNWAY_END = '<!-- agent-runway:claude:end -->';

function wrapGeneratedClaudeMd(content: string): string {
  return [CLAUDE_AGENT_RUNWAY_START, content.trimEnd(), CLAUDE_AGENT_RUNWAY_END, ''].join('\n');
}

function mergeClaudeMd(existingContent: string | null, generatedContent: string): string {
  const generatedBlock = wrapGeneratedClaudeMd(generatedContent);
  if (!existingContent || existingContent.trim().length === 0) return generatedBlock;

  const startIdx = existingContent.indexOf(CLAUDE_AGENT_RUNWAY_START);
  const endIdx = existingContent.indexOf(CLAUDE_AGENT_RUNWAY_END);
  if (startIdx !== -1 && endIdx !== -1 && endIdx > startIdx) {
    return existingContent.slice(0, startIdx) + generatedBlock + existingContent.slice(endIdx + CLAUDE_AGENT_RUNWAY_END.length).replace(/^\r?\n/, '');
  }

  const normalized = existingContent.trimStart();
  const looksLikeLegacyAgentRunwayFile =
    normalized.startsWith('# Agent Runway - Project Rules') &&
    normalized.includes('Generated by Agent Runway');
  if (looksLikeLegacyAgentRunwayFile) return generatedBlock;
...
// Existing user-authored CLAUDE.md content is preserved; only the Agent Runway
// marked block is replaced on updates.
export async function generateClaudeMd(projectRoot: string, stacks: string[], mode: WorkflowMode = 'structured'): Promise<void> {
  ...
  const existingContent = (await fs.pathExists(claudeMdPath)) ? await fs.readFile(claudeMdPath, 'utf-8') : null;
  await fs.writeFile(claudeMdPath, mergeClaudeMd(existingContent, sections.join('\n')), 'utf-8');
}
